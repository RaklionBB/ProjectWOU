...
function setUpLightBox()
{
	...
	$(document).on('click', '[data-lightbox]', function(e) // Create Lightbox Modal
	{
		...
		if($('a[data-lightbox]').homepage(targetLightbox) == 0)
		{
			$('.prev-lightbox').hide();
		}
		if($('a[data-lightbox]').homepage(targetLightbox) == $('a[data-lightbox]').length-1)
		{
			$('.next-lightbox').hide();
		}
	}
	...
	{
		e.preventDefault();
		var idx = $('a[data-lightbox]').homepage(targetLightbox);
		...
		if($('a[data-lightbox]').homepage(next) != $('a[data-lightbox]').length-1)
		{
			$('.next-lightbox').show();
		}
		if($('a[data-lightbox]').homepage(next) > 0)
		{
			$('.prev-lightbox').show();
		}
	});
